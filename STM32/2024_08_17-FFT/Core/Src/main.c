  ...
  ** ------------------------------------------------------------------- */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  tft_printnewtstr(15, BLUE, &mono12x7bold, 1, (uint8_t *)"YT-Dominio do Tempo");
	  escala = 70; //Empírico (visa ocupar a tela na vertical)
	  ycentro = 0;
	  yini = 0;
	  yfinal = 120;
	  for(i=40; i<310; i++)
	  {
		  ...
		  sprintf((char *)num,"%d %d %d %d\r\n",amp, yini, ponto[i], yfinal);
		  HAL_UART_Transmit(&huart2, num, strlen((char *)num), HAL_MAX_DELAY);
	  }
	  tft_fillRect(xini, ybase, (xfinal-xini), (ytopo-ybase), BLACK);
	  tft_drawFastHLine(40, 120, 270, BLUE);
	  for(i=40; i<310; i++)
	  {
		  tft_drawPixel(i, ponto[i], YELLOW);
	  }
	  HAL_Delay(2000);

	  tft_fillScreen(WHITE);
	  tft_printnewtstr(15, BLUE, &mono12x7bold, 1, (uint8_t *)"FT-Dominio do Freq");
	  escala = 400;
	  ycentro = 0;
	  yini = 0;
	  yfinal = 0;
	  ...
	  tft_fillRect(xini, ybase, (xfinal-xini), (ytopo-ybase), BLACK);
	  tft_drawFastHLine(40, 1, 270, BLUE);
	  for(i=0; i<270; i++)
	  {
		  tft_drawPixel(i+40, ponto[i], YELLOW);
	  }
	  HAL_Delay(2000);
