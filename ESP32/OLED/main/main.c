...
#define I2C_PIN_NUM_SDA           21
#define I2C_PIN_NUM_SCL           22
  ...
  */
  while(1) 
  {
    ...
    if ( r == ESP_OK ) 
    {
      ...
      draw_text(panel_handle, "Andre", 0, 0);
    } 
...
void draw_text(esp_lcd_panel_handle_t panel_handle, const char *text, int x, int y) 
{
    while (*text) 
    {
      ...
      for (int i = 0; i < 8; i++) 
      {
        uint8_t line = ssd1306_font8x8[c - 32][i];
        for (int j = 0; j < 8; j++) 
        {
          if (line & 1) 
          {
            esp_lcd_panel_draw_bitmap(panel_handle, x + i, y + j, x + i + 1, y + j + 1, (uint8_t[]){1});
          }
          line >>= 1;
        }
      }
